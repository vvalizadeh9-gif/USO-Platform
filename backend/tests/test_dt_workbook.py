...
def test_the_summary_agrees_with_the_dashboard(client, world):
    ...
    # "Assignment", the plan's figure -- distinct from the "Assigned" row in
    # the stage block, which is the workflow stage of the same name.
    assert summary["Assignment"] == plan["assigned"]
...
def test_no_two_summary_rows_share_a_label(client, world):
    """One label, one figure -- or a lookup down column A answers with the
    wrong one.

    The stage block and the plan block both used to carry a row called
    "Assigned", meaning two different things, and it went unnoticed for as
    long as both happened to be the same number. The parity test above reads
    the sheet into a dict keyed by label, so a collision there does not fail
    -- it shadows one figure with the other and quietly agrees.
    """
    ws = _workbook(client, world["admin"])["Summary"]
    labels = [
        row[0]
        for row in ws.iter_rows(min_row=1, max_col=2, values_only=True)
    ...
    assert not duplicates, f"Summary rows share a label: {sorted(duplicates)}"
